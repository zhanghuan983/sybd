    ...
    <view class="tab-content">
      <view v-if="activeTab === 'tab1'" class="tab-pane" :style="{ transform: 'translateX(' + (activeTab === 'tab1' ? '0' : '100%') + ')' }">
        ...
        <view class="txtdown"> DL/T 596-2021 标准文件：
          ...
          <view class="copy-link" @click="showRewardAd">文件下载1</view>
        </view>

        <!-- 页面内容 -->
        <view class="list-title">6、电力变压器</view>
        <!-- 列表项 -->
        <view class="list">
      ...
      <view
        v-if="activeTab === 'tab2'"
        class="tab-pane"
        :style="{ transform: 'translateX(' + (activeTab === 'tab2' ? '0' : '-100%') + ')' }"
      >
        <uv-notice-bar :text="listtxt2" mode="closable" customStyle=""></uv-notice-bar>
        <view class="txtdown2"> GB 50150-2016 标准文件：
          <view class="copy-link" @click="previewOnline" style="width: 140rpx;">在线预览</view>
...
export default {
  ...
  data() {
    return {
      ...
      previewLink: 'https://mp.weixin.qq.com/s/oQHEF91AUNDYEhLjiRTMxA',
      listItems: [
        { text: '6.1 油浸式电力变压器', url: '/pages/preview/preview?url=https://mp.weixin.qq.com/s/zFGy4LA0hz0CDoZYJAiKBw' },
        { text: '6.2 干式变压器、干式接地变压器', url: '/pages/preview/preview?url=https://mp.weixin.qq.com/s/cLMf1wAMUz8-j8v6CRuP4g' },
        { text: '6.3 SF6气体绝缘变压器', url: '/pages/preview/preview?url=https://mp.weixin.qq.com/s/Ce2cSLuZkr6zL3aae8eN6A' },
        { text: '6.4 判断故障时可供选用的试验项目见附录E', url: '/pages/preview/preview?url=https://mp.weixin.qq.com/s/TBkHO9gzGb3ISmvhKxMBQA' },
        // 你可以根据需要添加更多列表项
      ],
      videoAd: null
    };
